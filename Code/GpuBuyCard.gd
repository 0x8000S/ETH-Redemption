...
@export_storage var Gpu:GpuCard
func Init(gpu:Global.Gpu):
	...
	$MarginContainer/VBoxContainer/SubTitle.text = tr("基础算力: %sMH/s\nTDP: %sW\n显存: %sG\n定价: ￥%s") % [Gpu.BaseHashrate, Gpu.DefaultPower, Gpu.Vram, Gpu.MarketPrice]

func LoadInit(GPUC:GpuCard):
	Gpu = GPUC
	$MarginContainer/VBoxContainer/GpuName.text = Gpu.Model
	$MarginContainer/VBoxContainer/SubTitle.text = tr("基础算力: %sMH/s\nTDP: %sW\n显存: %sG\n定价: ￥%s") % [Gpu.BaseHashrate, Gpu.DefaultPower, Gpu.Vram, Gpu.MarketPrice]
